...
// import { createInlineLlamaIcon, formatPrice, getBatchTokenPrices, getStorage, getTokenPrice, logImage } from "../../libs/helpers";
import { formatPrice, getBatchTokenPrices, getStorage, getTokenPrice } from "../../libs/helpers";
...
export async function injectPrice(config: EtherscanAlikeExplorerConfig) {
	...
	async function renderMissingPricesInDropdownOnAddressPage() {
		const getERC20Items = () => getElements<HTMLAnchorElement>(["li.list-custom-ERC20 > a", "li.list-custom-ERC-20 > a"], document, true);
		...
		const cleanupSpam = () => {
			...
			if (!ulElement) return;
			listItems = getElements<HTMLAnchorElement>(["li > a"], ulElement, true);
		...
		const totalAmountContainer = getElements<HTMLElement>(["a#availableBalanceDropdown", "button#dropdownMenuBalance"]);
		if (!totalAmountContainer) return;
		const totalAmountTextNode = totalAmountContainer.childNodes[0];
		if (!totalAmountTextNode) return;
		const hasMoreTokens = totalAmountTextNode.textContent?.includes(">") ?? false;
		...
		for (const [address, { price, symbol }] of Object.entries(prices)) {
			console.log("testing price2", price, symbol)
			const listItem = addressItemMap[addressPriceMap[address]];
			if (listItem) {
				const amountSpan = getElements<HTMLSpanElement>(["span.list-amount", ".text-muted"], listItem);
				if (!amountSpan) continue;

				const amount = parseFloat(((amountSpan.textContent || "").trim().split(" ")[0].replace(/,/g, "")));
				const textRightDiv = getElements<HTMLDivElement>(["div.text-right", "div.text-end"], listItem);
				const usdValueSpan = getElements<HTMLSpanElement>([".list-usd-value"], listItem);
				if (!textRightDiv || !usdValueSpan) continue;
	...
	function tokenHasPrice(token: Element) {
		const usdValueSpan = getElements<HTMLElement>([".list-usd-rate"], token);
	...
	async function renderErc20PriceOnAddressPage() {
		...
		console.log("testing price", price, symbol)
...
function getElements<T extends Element>(
    ...
    if (isList) {
        for (const selector of selectors) {
            const nodeList = root.querySelectorAll<T>(selector);
